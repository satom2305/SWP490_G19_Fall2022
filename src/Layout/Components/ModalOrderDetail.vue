    ...
    <b-card id="update-order" class="main-card search-wrapper mb-20">
      <div class="mb-4">
        ...
        <div class="checkout__order">
          ...
          <ul v-for="(item, index) in getCurrentDetail" :key="index">
            <li>
              {{ item.productName }} x
              {{ item.quantity }}
...
export default {
  ...
  computed: {
    ...
    getTotalProductPrice() {
      return this.currentOrderDetail && this.currentOrderDetail.length > 0
        ? this.currentOrderDetail
            .map((item) => (item.productPrice ? item.productPrice : 0))
