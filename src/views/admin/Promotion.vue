<template>
  <div>
      ...
      <template v-else>
        ...
        <div class="text-right">
          <b-button
            variant="info"
            class="mb-4"
            @click="openModalUpdatePromotion(null, false)"
        ...
        <b-row v-if="getPromotions && getPromotions.length > 0">
          ...
          <b-col class="mt-1">
            <span class="text-muted">
              {{ fromPage }} đến {{ toPage }} trên
              {{ getPromotions.length }} bản ghi
            </span>
          </b-col>
        </b-row>
    ...
    <b-modal
      id="update-promotion"
      :title="isUpdatePromotion ? 'Cập nhật khuyến mại' : 'Thêm khuyến mại'"
      :no-close-on-backdrop="true"
      size="lg"
      @hidden="cancelUpdatePromotion"
    >
      <b-form-group>
        ...
        <b-form-input
          ...
          :class="{
            'is-invalid': validationStatus($v.currentData.promotionCode),
          }"
        ></b-form-input>
        <div
      ...
      <b-form-group>
        ...
        <b-form-input
          ...
          :class="{
            'is-invalid': validationStatus($v.currentData.salePercent),
          }"
        ></b-form-input>
        <div
        ...
        <div v-if="!$v.currentData.amount.maxValue" class="invalid-feedback">
      ...
      <b-form-group>
        ...
        <div v-if="!$v.currentData.amount.minValue" class="invalid-feedback">
    ...
    >
      <div class="pb-3">
        Bạn có muốn xoá khuyến mại<span
          class="font-weight-bold"
          v-if="this.currentPromotion"
          >{{ this.currentPromotion.promotionCode }}</span
        >
        không ?
      </div>
      <b-button
...
export default {
  ...
  methods: {
    ...
    changePage(e) {
      ...
      const dataSearch = this.$route.query.dataSearch;
      if (
        (dataSearch && dataSearch !== JSON.stringify(this.dataFilter)) ||
        !dataSearch
      ) {
        router.push({
    ...
    async handleUpdatePromotion(payload) {
      let errorMsg = this.isUpdatePromotion
        ? "Cập nhật khuyến mại không thành công."
        : "Thêm khuyến mại không thành công.";
      let successMsg = this.isUpdatePromotion
        ? "Cập nhật khuyến mại thành công."
        : "Thêm khuyến mại thành công.";
      let payloadForUpdate = {
        promotionId: this.currentData.promotionId,
        promotionData: payload,
      };
      let res = await (this.isUpdatePromotion
